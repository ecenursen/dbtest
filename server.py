...
from forms import FlaskForm, PatientSearchForm, LoginForm, G_PharmacySearchForm, HospitalSearchForm, HospitalAddForm,HospitalDeleteForm,inventory_change_form,G_WarehouseSearchForm,PharmacyPersonelForm,PharPersonelEditForm,PharPersonelAdd, delete_pharmacy_form,create_pharmacy_form
import datetime
import os
...
def pharmacy_page():
	...
	create_form=create_pharmacy_form()
	...
	if ((logged_in) and ( s )):
		...
		if delete_form.validate_on_submit():
			statement = "DELETE FROM public.pharmacies where id={}".format(phar_id)
			cursor.execute(statement)
			connection.commit()
		...
		cursor.close()
		return render_template('pharmacy_page.html', on_duty=on_duty, id=phar_id, Pharma=phar_detail, helper=helper, pharmacist=pharmacist, search_form=form1, logged_in=logged_in, forms=form2,add_form=add_form,delete_form=delete_form)
	...
	if (logged_in): #pharma or pwarehouse
		if (mode == 'p'):
			...
			if (self):
				...
				cursor.close()
				print (i)
				return render_template('inventory_page.html' , self=False , name = name, results = inventory, i=i,forms=forms)
